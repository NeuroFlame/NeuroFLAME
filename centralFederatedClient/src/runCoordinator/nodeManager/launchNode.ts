...
const launchSingularityNode = async ({
  ...
  try {
    ...
    // Add /tmp mount for compatibility (as in old implementation)
    ...
    // Pass through CI environment variable if set (for network configuration)
    ...
    const containerId = `singularity-${instanceProcess.pid}`
...
const findSingularityImage = async (imageName: string): Promise<string> => {
  ...
  // Search for images in current directory and common locations
  const searchPaths = [
    process.cwd(),
