...
test('accepts only allowlisted shared-error envelopes', () => {
  const valid =
    'NEUROFLAME_SHARED_ERROR:{"schema_version":1,' +
    '"origin":"site","stage":"transfer",' +
    '"code":"participant_computation_failed"}'
  assert.equal(
  ...
  assert.equal(
    extractSharedError(
      `ordinary log line\n2026-08-06T23:59:00.123456789Z ${valid}`,
      'fallback',
    ),
    'Participant computation failed during artifact transfer',
  )
